...
import {
    put,
    call,
    takeEvery, select, takeLatest, take
} from 'redux-saga/effects'

// Import all actions and api's
import {
    SET_LOADING,
    GET_SINGLE_FLASHCARD_REQUESTED,
    GET_FLASHCARD,
    GET_FLASHCARDS_REQUESTED,
    GET_SINGLE_FLASHCARD,
    GET_CREATE_FLASHCARD,
    CREATE_FLASHCARD,
...
function getID(state: { cards: { flashcards: string[]} }) {
    console.log('in ID', state.cards.flashcards[0])
...
export function* getSingleFlashcard(): any {
    //SECOND TIME I CAN'T START THIS FUNC
...
// Create Flashcard
function* createFlashcard(action: any): any { // check how fix this any with TL Michał
